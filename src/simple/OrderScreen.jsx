...
function Draft({ vendor, inventory, stock, user, canOrder, placeOrder }) {
  ...
  async function send() {
    ...
    setBusy(false);
    setResult(saved.ok ? "placed" : "local");
    setPhase("done");
  }
  ...
  if (phase === "done") {
    return (
      <div className="card">
        <h2>{result === "placed" ? "Order placed" : "Order saved on this device"}</h2>
        ...
        <div className="stack block">
          ...
          <button type="button" className="btn quiet" onClick={() => setPhase("edit")}>Edit another order</button>
        </div>
      </div>
    );
  }
